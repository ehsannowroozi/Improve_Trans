...
Find Best Epsilon - more inside the region - Best Step - Find Optimum Step
Find Best Epsilon - more inside the region - Best Step - Find Optimum Step
Find Best Epsilon - more inside the region - Best Step - Find Optimum Step
Find Best Epsilon - more inside the region - Best Step - Find Optimum Step
Find Best Epsilon - more inside the region - Best Step - Find Optimum Step
...
class Adversarial(object):
    ...
    def _reset(self):
        ...
        ''' modified part by Ehsan '''
        self.__best_epsilon = None
        self.__epsilons_been_used = []
        self.__attack_gradient = None
        self.__best_prediction_value = self.__ori_prediction_value
        self.__psnr = None
        ''' modification ended'''
    ...
    ''' added part by Ehsan '''
    @property
    def best_epsilon(self):
        return self.__best_epsilon

    @property
    def attack_gradient(self):
        return self.__attack_gradient

    @attack_gradient.setter
    def attack_gradient(self, value):
        self.__attack_gradient = value

    @property
    def epsilons_been_used(self):
        return self.__epsilons_been_used

    @epsilons_been_used.setter
    def epsilons_been_used(self, value):
        self.__epsilons_been_used.append(value)

    @property
    def ori_prediction_value(self):
        return self.__ori_prediction_value

    @property
    def best_prediction_value(self):
        return self.__best_prediction_value
    ...
    def __new_adversarial(self, image, predictions, in_bounds, epsilon, idx_step):
        def dis(a, b):
          return np.mean((a-b)**2, dtype=np.float64)

        def psnr(image):
            mse = np.mean((self.__original_image - image) ** 2)
            if mse == 0:
                return 100
            PIXEL_MAX = 1.0
        ...
        def max_dist(image):
             diff = np.double(self.__original_image) - np.double(image)
             return 255*abs(diff).max()
        ...
        new_prediction = self.bowen_prediction(image)
        ori_prediction = self.ori_prediction_value
        best_prediction = self.best_prediction_value
        self.__psnr = psnr(image)
        #if in_bounds and dis(ori_prediction, best_prediction) < dis(ori_prediction, new_prediction):
        #if in_bounds and dis(ori_prediction, best_prediction) < dis(ori_prediction, new_prediction) \
        #  and psnr(image) > 30 and max_dist(image) < 10:
        if in_bounds and dis(ori_prediction, best_prediction) < dis(ori_prediction, new_prediction) \
            and psnr(image) > 40:
            ''' modification ended '''
            ...
            # self.__best_epsilon = epsilon
            self.__best_epsilon = [epsilon,idx_step]
            ...
            ''' modified by bowen'''
            # update the best
            self.__best_prediction_value = new_prediction
            '''modification ended'''
    ...
    def __is_adversarial(self, image, predictions, in_bounds, epsilon, idx_step):
        ...
        if is_adversarial:
            is_best, distance = self.__new_adversarial(
                image, predictions, in_bounds, epsilon, idx_step)
        else:
            is_best = False
            distance = None
    ...
    def predictions(self, image, epsilon = None, idx_step = None, strict=True, return_details=False):
        ...
        is_adversarial, is_best, distance = self.__is_adversarial(
            image, predictions, in_bounds, epsilon, idx_step=idx_step)
    ...
    ''' added by bowen'''
    def bowen_prediction(self, image, ):
        return self.__model.predictions(image)
    ''' added part end'''
